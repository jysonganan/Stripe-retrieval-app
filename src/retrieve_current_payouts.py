...
def retrieve_current_payouts(current_month = '01', current_year = '2023', api_key):
    stripe.api_key = api_key
    
    arrival_dates = [datetime.datetime.fromtimestamp(stripe.Payout.list(limit = 100)['data'][i]['arrival_date']).strftime("%m/%d/%Y") for i in range(len(stripe.Payout.list()['data']))]
