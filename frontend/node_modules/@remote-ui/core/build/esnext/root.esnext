import { KIND_ROOT, ACTION_MOUNT, ACTION_INSERT_CHILD, KIND_TEXT, ACTION_REMOVE_CHILD, KIND_COMPONENT, ACTION_UPDATE_PROPS, ACTION_UPDATE_TEXT, KIND_FRAGMENT } from './types.esnext';
...
function appendChild(container, child, internals, rootInternals) {
  ...
  return perform(container, rootInternals, {
    remote: channel => channel(ACTION_INSERT_CHILD, container.id, container.children.length, serializeChild(child)),
    local: () => {
...
function insertChildBefore(container, child, before, internals, rootInternals) {
  ...
  return perform(container, rootInternals, {
    remote: channel => channel(ACTION_INSERT_CHILD, container.id, container.children.indexOf(before), serializeChild(child)),
    local: () => {
      moveNodeToContainer(container, child, rootInternals);
      const newChildren = [...internals.children];
