...
def retrieve_current_payouts(api_key, current_month='', current_year=''):
    ...
    arrival_dates = [
        datetime.datetime.fromtimestamp(stripe.Payout.list(limit=100)['data'][i]['arrival_date']).strftime("%m/%d/%Y")
        for i in range(len(stripe.Payout.list(limit=100)['data']))]
    ...
    current_pay_out_ids = [stripe.Payout.list(limit=100)['data'][i]['id'] for i in current_pay_out_ids[0]]
